...
class TestWriteAllowed:
    ...
    def test_hermes_credential_files_are_write_denied(self):
        from hermes_constants import get_hermes_home

        home = get_hermes_home()
        # config.yaml is gated by _check_sensitive_path, not this denylist.
        assert _is_write_denied(str(home / "config.yaml")) is False
        for name in ["auth.json", "auth.lock", "webhook_subscriptions.json"]:
            assert _is_write_denied(str(home / name)) is True, f"{name} should be denied"
